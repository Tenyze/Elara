...
public class ColorSliderComponent implements Component {
    ...
    public void draw(java.util.concurrent.atomic.AtomicInteger offset) {
        ...
        GL11.glPopMatrix();
        if (!draggingHue && !draggingSat && !draggingBri && !draggingAlpha) {
        ...
        int previewColor = (int) (alpha * 255) << 24 | (Color.HSBtoRGB(hue, saturation, brightness) & 0xFFFFFF);
        ...
        // Alpha bar: transparent -> opaque
        drawGradientRect(x, alphaY, x + width, alphaY + 4,
                new Color(Color.HSBtoRGB(hue, saturation, brightness)).getRGB() & 0x00FFFFFF,
                Color.HSBtoRGB(hue, saturation, brightness));
        drawPointer(x, alphaY, width, alpha);
    }
    ...
    public void mouseDown(int mouseX, int mouseY, int button) {
        ...
        else if (isHovered(mouseX, mouseY, baseY + (4 + 2) * 3)) draggingAlpha = true;
    }
