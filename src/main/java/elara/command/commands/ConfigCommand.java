    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            ...
            ChatUtil.sendFormatted(
                    String.format("%sUsage: .%s &oload&r/&osave&r <&oname&r> | .%s &olist&r | .%s &ofolder&r | .%s &oreset&r", Elara.clientName, command, command, command, command)
            );
        } else {
            ...
            switch (sub) {
                ...
                case "reload":
                    if (args.size() < 3) {
                        ChatUtil.sendFormatted(
                                String.format("%sMissing config name (use '&odefault&r' or '&o!&r' to load default config)&r", Elara.clientName)
                        );
                        return;
                    }
                ...
                case "restore":
                case "defaults":
                    boolean del = args.size() >= 3 && ("-f".equalsIgnoreCase(args.get(2)) || "force".equalsIgnoreCase(args.get(2)) || "delete".equalsIgnoreCase(args.get(2)));
                    Config.resetAll(del);
                    return;
                case "list":
                    try {
                        File[] configs = new File("./config/elara/").listFiles(FILE_FILTER);
                        if (configs == null) {
                            throw new Exception();
                        }
                        if (configs.length == 0) {
                            ChatUtil.sendFormatted(String.format("%sNo configs found (&o%s&r)&r", Elara.clientName, "./config/elara/"));
                        }
                        Arrays.sort(configs, LastModifiedFileComparator.LASTMODIFIED_REVERSE);
                        ChatUtil.sendFormatted(String.format("%sConfigs:&r", Elara.clientName));
                        ...
                        ChatUtil.sendFormatted(String.format("%sFailed to read (&o%s&r)&r", Elara.clientName, "./config/elara/"));
                    }
                    return;
                case "f":
                case "folder":
                ...
                case "directory":
                    try {
                        Desktop.getDesktop().open(new File("./config/elara/"));
                    } catch (Exception e) {
                        ChatUtil.sendFormatted(String.format("%sFailed to open (&o%s&r)&r", Elara.clientName, "./config/elara/"));
                    }
                    return;
                default:
                    ChatUtil.sendFormatted(String.format("%sInvalid argument (&o%s&r)&r", Elara.clientName, args.get(1)));
            }
        }
    }
}
