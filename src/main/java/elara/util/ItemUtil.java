...
public class ItemUtil {
    ...
    public static boolean isNotSpecialItem(ItemStack itemStack) {
        ...
        // 钓鱼竿不在此特殊处理，交给调用方(如InvManager的keepFishingRod)判断
    ...
    public static double getArmorProtection(ItemStack itemStack) {
        ...
        if (itemStack != null) {
            if (itemStack.getItem() instanceof ItemArmor) {
                ...
                // 材质优先级：钻石(4) > 铁(3) > 金(2) > 皮革(1)
                protection = getArmorMaterialPriority(armor.getArmorMaterial()) * 10.0;
    ...
    private static double getArmorMaterialPriority(ItemArmor.ArmorMaterial material) {
        switch (material) {
            case DIAMOND: return 4.0;
            case IRON: return 3.0;
            case GOLD: return 2.0;
            case LEATHER: return 1.0;
            default: return 0.0;
        }
    }
    ...
    public static boolean hasRawUnbreakingEnchant() {
        if (ItemUtil.mc == null || ItemUtil.mc.thePlayer == null) {
            return false;
        }
        ...
        if (itemStack.hasTagCompound()) {
            ...
            if (tag != null) {
                if (tag.hasKey("ExtraAttributes")) {
                    ...
                    if (extra != null && extra.hasKey("UHCid")) {
        ...
        try {
            ...
            if (enchants != null && enchants.containsKey(19)) {
                return true;
            }
    ...
    public static boolean isTNT(ItemStack itemStack) {
        if (itemStack == null) return false;
        Item item = itemStack.getItem();
        if (item instanceof ItemBlock) {
            Block block = ((ItemBlock) item).getBlock();
            return block == net.minecraft.init.Blocks.tnt;
        }
        return false;
    }
}
