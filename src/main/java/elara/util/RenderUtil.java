...
public class RenderUtil {
    ...
    public static void drawRoundedRectangle(float x, float y, float x2, float y2, float radius, final int color) {
        ...
        radius = Math.min(radius, Math.min(width / 2.0f, h / 2.0f));
        if (radius <= 0.001f) {
        ...
        GL11.glBegin(GL11.GL_POLYGON);
        glColor(color);
        final double step = 2.0 * Math.PI / 72.0; // 72 segments per full circle
        // Top-left corner (180° → 270°)
        for (double a = Math.PI; a < Math.PI * 1.5 + step * 0.5; a += step) {
            GL11.glVertex2d(x + r + Math.cos(a) * r, y + r + Math.sin(a) * r);
        }
        // Top-right corner (270° → 360°)
        for (double a = Math.PI * 1.5; a < Math.PI * 2.0 + step * 0.5; a += step) {
            GL11.glVertex2d(x2 - r + Math.cos(a) * r, y + r + Math.sin(a) * r);
        }
        // Bottom-right corner (0° → 90°)
        for (double a = 0.0; a < Math.PI * 0.5 + step * 0.5; a += step) {
            GL11.glVertex2d(x2 - r + Math.cos(a) * r, y2 - r + Math.sin(a) * r);
        }
        // Bottom-left corner (90° → 180°)
        for (double a = Math.PI * 0.5; a < Math.PI + step * 0.5; a += step) {
            GL11.glVertex2d(x + r + Math.cos(a) * r, y2 - r + Math.sin(a) * r);
        }
        GL11.glEnd();
