...
public class InvManager extends Module {
    ...
    // 保留项：开启后对应物品即使未匹配 hotbar 槽位也不会被 drop-trash 丢掉
    public final BooleanProperty keepFishingRod = new BooleanProperty("keep-fishing-rod", true);
    public final BooleanProperty keepTNT = new BooleanProperty("keep-tnt", true);
    public final BooleanProperty keepBowAndArrow = new BooleanProperty("keep-bow-arrow", true);
                            ...
                            for (int i = 0; i < 36; i++) {
                                if (!equippedArmorSlots.contains(i)
                                    ...
                                    if (stack != null) {
                                        ...
                                        // ===== 保留项检查 =====
                                        if (keepFishingRod.getValue() && ItemUtil.isFishingRod(stack)) continue;
                                        if (keepTNT.getValue() && ItemUtil.isTNT(stack)) continue;
                                        if (keepBowAndArrow.getValue()) {
                                            if (ItemUtil.isBow(stack) || ItemUtil.isArrow(stack)) continue;
                                        }
                                        if (isBlock) {
