...
public class AimAssist extends Module {
    ...
    private boolean isValidTarget(EntityPlayer entityPlayer) {
        if (entityPlayer != mc.thePlayer && entityPlayer != mc.thePlayer.ridingEntity) {
            if (entityPlayer == mc.getRenderViewEntity() || entityPlayer == mc.getRenderViewEntity().ridingEntity) {
                ...
                return (!this.team.getValue() || !TeamUtil.isSameTeamAdvanced(entityPlayer)) && (!this.botChecks.getValue() || !TeamUtil.isBot(entityPlayer));
            }
        } else {
            return false;
        }
    ...
    private boolean isInReach(EntityPlayer entityPlayer) {
        Reach reach = (elara.module.combat.Reach) Elara.moduleManager.modules.get(elara.module.combat.Reach.class);
    ...
    public void onPress(KeyEvent event) {
        if (event.getKey() == mc.gameSettings.keyBindAttack.getKeyCode() && !Elara.moduleManager.modules.get(elara.module.utility.AutoClicker.class).isEnabled()) {
            this.timer.reset();
        }
    }
}
