...
public class HitSelect extends Module {
    ...
    private void fixMotion() {
        ...
        // 修复：不再自动启用KeepSprint模块，直接保持motion避免被反作弊检测
        // 如果KeepSprint已启用，由KeepSprint处理；否则保持当前motion
        KeepSprint keepSprint = (KeepSprint) Elara.moduleManager.modules.get(KeepSprint.class);
        if (keepSprint != null && keepSprint.isEnabled()) {
            // KeepSprint已启用，无需额外处理
            this.set = true;
            return;
        }
        // 直接保持当前motion，避免攻击时被减速
        this.set = true;
    }
