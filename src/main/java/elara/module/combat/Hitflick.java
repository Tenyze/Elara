...
public class Hitflick extends Module {
    ...
    private BlinkModules BlinkModules;
    ...
    public Hitflick() {
        ...
        direction.setCategory("Rotation");
        customAngle.setCategory("Rotation");
        cooldown.setCategory("Timing");
        blink.setCategory("Other");
    }
    ...
    public void onDisabled() {
        ...
        sinceLastFlick = 0;
        if (blink.getValue()) {
            Elara.blinkManager.setBlinkState(false, BlinkModules);
        }
    }
    ...
    public void onAttack(AttackEvent event) {
        ...
        if (blink.getValue()) {
            Elara.blinkManager.setBlinkState(false, Elara.blinkManager.getBlinkingModule());
            Elara.blinkManager.setBlinkState(true, BlinkModules.HITFLICK);
        }
    }
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (state == FlickState.FLICKING_AWAY) {
            ...
            sinceLastFlick = 0;
            if (blink.getValue()) {
                Elara.blinkManager.setBlinkState(false, BlinkModules.HITFLICK);
            }
        }
    }
