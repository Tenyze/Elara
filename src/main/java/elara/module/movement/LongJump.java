...
public class LongJump extends Module {
    ...
    public final ModeProperty mode = new ModeProperty("mode", 0, new String[]{"FIREBALL", "FIREBALL_MANUAL", "FIREBALL_HIGH", "FIREBALL_FLAT", "LEGIT"});
    ...
    public boolean isAutoMode() {
        return this.mode.getValue() == 0 || this.mode.getValue() == 2 || this.mode.getValue() == 3 || this.mode.getValue() == 4;
    }
        ...
        if (this.isEnabled() && event.getType() == EventType.PRE) {
            if (this.isLongJumpMode() && this.isJumping) {
                ...
                if (this.tickCounter == 1) {
                    switch (this.mode.getValue()) {
                        case 0:
                        case 1:
                        case 4: // LEGIT: 使用默认阶段，由knockback自然推动
            ...
            if (this.isAutoMode() && !this.isJumping) {
                ...
                if (this.mode.getValue() == 4) {
                    // LEGIT: 优化角度，垂直向下丢火球，保持移动方向获得向前knockback
                    yaw = RotationUtil.quantizeAngle(mc.thePlayer.rotationYaw + RandomUtil.nextFloat(-0.5F, 0.5F));
                    pitch = RotationUtil.quantizeAngle(90.0F + RandomUtil.nextFloat(-0.1F, 0.1F));
                } else {
                    yaw = RotationUtil.quantizeAngle(mc.thePlayer.rotationYaw - 180.0F - RandomUtil.nextFloat(0.0F, 1.0F));
                    pitch = RotationUtil.quantizeAngle(89.0F + RandomUtil.nextFloat(-0.25F, 0.25F));
                }
    ...
    public void onEnabled() {
        ...
        if (this.isAutoMode() && this.findFireballInHotbar() == -1) {
            this.setEnabled(false);
            ChatUtil.sendFormatted(String.format("%s%s: &cNo fireball found in your hotbar!&r", Elara.clientName, this.getName()));
        }
    }
