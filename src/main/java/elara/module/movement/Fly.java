...
public class Fly extends Module {
    ...
    private int polarAttackCounter = 0;
    ...
    private void updatePolarState() {
        ...
        if (this.polarAttackedBoat && !this.polarMovementCheckDisabled) {
            if (this.isPolarBoatGone()) {
                this.polarMovementCheckDisabled = true;
            } else if (this.polarAttackCounter < 20) {
                PacketUtil.sendPacket(new C02PacketUseEntity(this.polarBoat, C02PacketUseEntity.Action.ATTACK));
                PacketUtil.sendPacket(new C0APacketAnimation());
                this.polarAttackCounter++;
            }
        }
    }
