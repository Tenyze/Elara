...
public class BedBreaker extends Module {
    ...
    private Color getProgressColor(int mode) {
        switch (mode) {
            ...
            case 2:
                return ((HUD) Elara.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis());
            default:
                return new Color(-1);
        }
    }
    ...
    public void onTick(TickEvent event) {
        if (event.getType() == EventType.PRE) {
            ...
            AutoBlockIn autoBlockIn = (AutoBlockIn) Elara.moduleManager.modules.get(AutoBlockIn.class);
            ...
            if (this.targetBed == null) {
                Elara.delayManager.setDelayState(false, DelayModules.BED_BREAKER);
            }
        }
    }
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        if (this.isEnabled()) {
            if (this.isBreaking()
                    && !Elara.playerStateManager.attacking
                    && !Elara.playerStateManager.digging
                    && !Elara.playerStateManager.placing
                    && !Elara.playerStateManager.swinging) {
                this.doSwing();
            }
        }
    }
    ...
    public void onRender3D(Render3DEvent event) {
        if (this.isEnabled() && this.targetBed != null && !mc.theWorld.isAirBlock(this.targetBed)) {
            ...
            if (this.showTarget.getValue() != 0) {
                BedESP bedESP = (BedESP) Elara.moduleManager.modules.get(BedESP.class);
                ...
                int b = color.getBlue();
                RenderUtil.drawBlockBox(target, newHeight, r, g, b);
    ...
    public void onPacket(PacketEvent event) {
        if (!event.isCancelled()) {
            ...
            if (this.isEnabled() && this.targetBed != null && this.ignoreVelocity.getValue() == 2 && Elara.delayManager.getDelayModule() != DelayModules.BED_BREAKER) {
                if (event.getPacket() instanceof S12PacketEntityVelocity) {
                    S12PacketEntityVelocity packet = (S12PacketEntityVelocity) event.getPacket();
                    if (packet.getEntityID() == mc.thePlayer.getEntityId() && packet.getMotionY() > 0) {
                        Elara.delayManager.delay(DelayModules.BED_BREAKER);
                ...
                if (event.getPacket() instanceof S27PacketExplosion) {
                    ...
                    if (explosion.func_149149_c() != 0.0F || explosion.func_149144_d() != 0.0F || explosion.func_149147_e() != 0.0F) {
                        Elara.delayManager.delay(DelayModules.BED_BREAKER);
                        Elara.delayManager.delayedPacket.offer(explosion);
                        event.setCancelled(true);
                    }
                }
            }
